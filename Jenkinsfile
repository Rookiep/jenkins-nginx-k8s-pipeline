pipeline {
    ...
    environment {
        ...
        GIT_REPO = "https://github.com/Rookiep/jenkins-nginx-k8s-pipeline.git"  // Update with real Git URL
    }
    stages {
        ...
        stage('Update Git Repo') {
            steps {
                sh '''
                    ...
                    git push origin main || true  # Add credentials if private repo
                '''
            }
        }
    }
    post { 
        success { echo 'Pipeline succeeded! Docker image pushed and Git updated. Argo CD will auto-sync.' } 
        always { sh 'docker rmi ${DOCKER_IMAGE} || true; rm -rf temp-repo' }  // Cleanup
    }
}